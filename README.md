...
Реальные API-ключи не должны попадать в git. Для локального запуска используйте `.env`, он игнорируется репозиторием.

Загруженные пользователем документы хранятся в `data/storage`, а состояние проектов — в `data/state`. В Docker эти директории вынесены в volumes, чтобы данные не терялись при пересборке образов.
